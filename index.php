...
$total_sales_count = $pdo->query("SELECT COUNT(*) FROM sales")->fetchColumn();

// 2. Revenue Analytics
$total_revenue = $pdo->query("SELECT SUM(total_amount) FROM sales")->fetchColumn() ?? 0;

// 3. Debt (Piutang) Analytics
$total_credit_sales = $pdo->query("SELECT SUM(total_amount) FROM sales WHERE payment_type = 'credit'")->fetchColumn() ?? 0;
$total_debt_paid = $pdo->query("SELECT SUM(amount_paid) FROM debt_payments")->fetchColumn() ?? 0;
...
$cash_sales = $pdo->query("SELECT SUM(total_amount) FROM sales WHERE payment_type != 'credit'")->fetchColumn() ?? 0;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(ctxYearly, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Revenue',
                data: yearlyRaw.map(d => d.revenue),
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            <?php
                            $recent_stmt = $pdo->query("SELECT * FROM sales ORDER BY created_at DESC LIMIT 5");
